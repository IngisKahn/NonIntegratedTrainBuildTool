...
int main() {
	...
	while (PROGRAMLOOP) {
		...
		std::cout << "⠀⠀⣿⣿⣜⣂⣼⣿⣿⣿⣿⣿⣇⣓⣼⣿⣿⣿⠃⢀⠀⣼⣿⠇⠀⠀⠀⣸⣿⣿⡿⠀⠀⠀⠘⣿⣿⣿⣿⠇⠀⣸⣿⣿⣿⠀⠀⠀⠀⠀⣿⣿⣿⡏⠀⠀⠀⠀⢰⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⠿⠁⠀⠀⠀⣼⣿⣿⡿⠀⠀⠀⠀⠀⠀⠀⠀\n";
		std::cout << "⠀⠀⠹⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣾⣿⡿⠀⠀⠀⠀⠛⠛⠛⠃⠀⠀⠀⠀⠛⠛⠛⠛⠀⠀⠛⠛⠛⠃⠀⠀⠀⠀⠘⠛⠛⠛⠁⠀⠀⠀⠀⠚⠛⠛⠛⠛⠛⠛⠛⠛⠋⠁⠀⠀⠀⠀⠐⠛⠛⠛⠃⠀⠀⠀⠀⠀⠀⠀⠀\n";
		std::cout << "⠀⠀⠀⠈⠛⠛⠛⠛⠛⠛⠛⠛⠛⠛⠛⣛⣿⣿⣿⣿⣿⣿⣿⣶⣦⣤⣤⣤⣀⣀⣀⠀⠀⠀⠀⠀⠀⠀NON-INTEGRATED TRAIN BUILD TOOL⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀\n";
		std::cout << "⠀⣠⣴⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⡿⠿⠛⠛⠛⠋⠉⠉⠉⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀by Hvxlo v1.02⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀ \n";
		std::cout << "⠈⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠁⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀          \n\n";
		...
		std::string trainsymbol;

		bool INTERMODAL = false;
		bool PROCEEDSYMBOL = false;

		selectedRailroad->DisplayLogo();

		while (!PROCEEDSYMBOL) {
			std::cout << "Enter the train symbol you are working on: ";
			std::getline(cin, trainsymbol);

			if (RRCHOICE == "UP") {
				if (!trainsymbol.empty()) {
					//transform(trainsymbol.begin(), trainsymbol.end(), trainsymbol.begin(), toupper);
					PROCEEDSYMBOL = true;
				}
			}
			else if (RRCHOICE == "NS") {
				if (!trainsymbol.empty() && std::isdigit(trainsymbol[0])) {
					int MORI = trainsymbol[0] - '0';
